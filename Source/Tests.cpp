...
int main(/*int argc, const char *argv[]*/)
{
    ...
    try
    {
        if (UnitTests::DOUBLYLINKEDLIST::DoublyLinkedListUnitTests())
        {
...
void TerminateCall()
{
    if(!CORRECTLY_FINISHED)
    {
        ...
        std::cout << "\tPlease note: This error also show up on ctrl-c and other \"early exit\" methodologies. Those are a-okay!" << std::endl;
        CORRECTLY_FINISHED = true;
        exit(1);
    }
}
